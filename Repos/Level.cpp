...
Level::Level(int stage, PlaySideBar* sideBar, Player* player) {
	...
	mStageLabel = new Texture("STAGE", "Minecraft.ttf", 32, { 75, 75, 200 });
	...
	mReadyLabel = new Texture("GO", "Minecraft.ttf", 48, { 150, 0, 0 });
	...
	mGameOverLabel = new Texture("GAME OVER", "Minecraft.ttf", 48, { 200, 0, 0 });
...
void Level::HandleEnemySpawning() {
				...
				if (child != NULL) {
					...
					if (type.compare("Butterfly") == 0) {
						if (!mChallengeStage) {
							...
							mEnemies.push_back(new Butterfly(index, path, true));
						}
					}
						...
						if (!mChallengeStage) {
							...
							mEnemies.push_back(new Wasp(index, path, true, false));						
						}
					}
						...
						if (!mChallengeStage) {
							...
							mEnemies.push_back(new Boss(index, path, true));
						}
					}
	...
	/*
	if (InputManager::Instance()->KeyPressed(SDL_SCANCODE_H) && mButterflyCount < MAX_BUTTERFLIES) {
		mEnemies.push_back(new Butterfly(mButterflyCount, 1, false));
		mButterflyCount++;
	}
	
	if (InputManager::Instance()->KeyPressed(SDL_SCANCODE_J) && mWaspCount < MAX_WASPS) {
		mEnemies.push_back(new Wasp(mWaspCount, 0, false, false));
		mWaspCount++;
	}

	if (InputManager::Instance()->KeyPressed(SDL_SCANCODE_K) && mBossCount < MAX_BOSSES) {
		mEnemies.push_back(new Boss(mBossCount, 3, false));
		mBossCount++;
	}
	*/
}
